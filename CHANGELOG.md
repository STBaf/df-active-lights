...
## Release 3.0.0 (2026-09-17) (Requires v14, unofficial port)
- **UPDATE:** Migrated to Foundry VTT v14.
- **UPDATE:** Active Light Config rewritten as `ApplicationV2` (Handlebars), no jQuery anymore.
- **UPDATE:** Uses the namespaced API (`foundry.canvas.placeables.AmbientLight`, `foundry.applications.handlebars.renderTemplate`, ...).
- **UPDATE:** Scene control toggle uses the v13+ `controls`/`tools` records and `onChange`.
- **UPDATE:** libWrapper is no longer required (only used by an optional fallback, if installed).
- **UPDATE:** Animations of the light preview (while the light config is open) use the stored animation data.
- **FIX:** Colour values below `#100000` were written without leading zeros.
- **FIX:** New key frames start with the light's current values instead of 0 / black.
- **FIX:** Invalid time input is visibly reset; the "already exists" warning shows seconds.
- **FIX:** Removing all animations resets the light to its configured values right away.
- **NEW:** German localization.

## Release 2.0.0 (2025-03-09)
- **UPDATE:** Migrated to v12.
- **UPDATE:** Downgraded TS -> JS (such sad).
- **NEW #379:** Add ability to manually control when an animation starts by toggling a light on/off.
